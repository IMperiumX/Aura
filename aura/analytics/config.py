...
class AnalyticsConfig:
    ...
    def _load_configuration(self) -> Dict[str, Any]:
        ...
        logger.warning(f"Unknown environment '{self.environment}', falling back to development config")
    ...
    def _validate_configuration(self) -> None:
        ...
        if not self.config['backends']:
            raise ImproperlyConfigured("Analytics configuration has no backends defined")
        ...
        for backend in self.config['backends']:
            ...
            if 'class' not in backend:
                raise ImproperlyConfigured(f"Backend '{backend['name']}' missing 'class'")
        ...
        if primary:
            ...
            if primary not in backend_names:
                raise ImproperlyConfigured(f"Primary backend '{primary}' not found in backends list")
    ...
    def validate_environment_requirements(self) -> Dict[str, bool]:
        ...
        for backend in self.get_backends_list():
            ...
            if 'pubsub' in backend_class.lower():
                # PubSub requirements
                requirements[f"{backend_name}_gcp_project"] = bool(options.get('project'))
    ...
    def is_production_ready(self) -> bool:
        ...
        if self.environment == 'production':
            ...
            if len(backends) < 2:
                logger.warning("Production should have multiple backends for redundancy")
                return False
    ...
    def get_metrics_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'aggregation_intervals': self.config.get('aggregation_intervals', ['hour', 'day']),
