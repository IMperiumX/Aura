...
from aura.analytics.api.serializers import AlertInstanceSerializer
from aura.analytics.api.serializers import AlertRuleSerializer
from aura.analytics.api.serializers import AnalyticsQuerySerializer
from aura.analytics.api.serializers import DashboardConfigSerializer
from aura.analytics.api.serializers import DashboardWidgetSerializer
from aura.analytics.api.serializers import LiveMetricsSerializer
from aura.analytics.api.serializers import SystemStatusSerializer
...
class DashboardWidgetViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = DashboardWidget.objects.filter(
            Q(created_by=user) | Q(is_public=True) | Q(allowed_users=user),
        ).distinct()
    ...
    def data(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {"error": f"Failed to get widget data: {e!s}"},
                ...
                formatted_events.append(
                    {
                        ...
                        "user_id": event.get("user_id"),
                        "ip_address": event.get("ip_address", "")[:8] + "..."
    ...
    def _get_top_events_data(self, filters: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            # Sort by count
            top_events = sorted(event_counts.items(), key=lambda x: x[1], reverse=True)[
                :10
            ]
    ...
    def get(self, request):
        ...
        try:
            ...
            if time_window == "hour" and response_data["total_events"] > 0:
                response_data["events_per_minute"] = (
                    response_data["total_events"] / 60.0
                )
            ...
            return Response(
                {"error": f"Failed to get live metrics: {e!s}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
