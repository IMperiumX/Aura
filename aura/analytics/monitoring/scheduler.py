...
from aura.analytics.models import AlertRule, MetricsSnapshot
...
def generate_metrics_snapshot_task(self, aggregation_type='hourly'):
    ...
    try:
        ...
        if existing:
            logger.info(f"Snapshot for {aggregation_type} {period_start} already exists")
            return {
                'status': 'exists',
        ...
        logger.info(f"Generated {aggregation_type} snapshot {snapshot.id} for period {period_start} to {period_end}")
...
def create_metrics_snapshot(period_start: datetime, period_end: datetime, aggregation_type: str) -> MetricsSnapshot:
    ...
    system_metrics = {
        'events_per_hour': len(events) / max(1, (period_end - period_start).total_seconds() / 3600),
    ...
    custom_metrics = {
        'patient_events': sum(1 for event in events if 'patient' in event.get('event_type', '').lower()),
        'therapy_events': sum(1 for event in events if 'therapy' in event.get('event_type', '').lower()),
        'communication_events': sum(1 for event in events if 'message' in event.get('event_type', '').lower()),
        'error_events': sum(1 for event in events if 'error' in event.get('event_type', '').lower()),
    }
