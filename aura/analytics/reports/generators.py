...
from aura.analytics.models import MetricsSnapshot, AlertInstance
...
class BaseReportGenerator:
    ...
    def _generate_pdf(self) -> Tuple[bytes, str]:
        ...
        try:
            ...
            html_content = render_to_string('analytics/reports/base_report.html', {
                'config': self.config,
                'data': self.data,
                'generated_at': timezone.now()
            })
    ...
    def _add_summary_sheet(self, workbook):
        ...
        # Period
        ws['A3'] = "Report Period:"
        ws['B3'] = f"{self.config.period_start.strftime('%Y-%m-%d')} to {self.config.period_end.strftime('%Y-%m-%d')}"
...
class AnalyticsSummaryReportGenerator(BaseReportGenerator):
    ...
    def _calculate_trends(self, snapshots):
        ...
        if first.total_events > 0:
            events_change = ((last.total_events - first.total_events) / first.total_events) * 100
            trends['events_trend'] = round(events_change, 2)

        if first.unique_users > 0:
            users_change = ((last.unique_users - first.unique_users) / first.unique_users) * 100
    ...
    def _analyze_user_activity(self, events):
        ...
        return {
            ...
            'avg_sessions_per_user': round(len(user_sessions) / max(1, len(user_events)), 2)
        }
    ...
    def _analyze_system_performance(self, snapshots):
        ...
        return {
            ...
            'performance_stability': round((1 - (max_events - min_events) / max(1, avg_events_per_period)) * 100, 2)
        }
...
class BusinessIntelligenceReportGenerator(BaseReportGenerator):
    ...
    def _analyze_patient_metrics(self, events):
        ...
        patient_events = [e for e in events if 'patient' in e.get('event_type', '').lower()]

        metrics = {
            'total_patient_events': len(patient_events),
            'new_patients': len([e for e in patient_events if 'created' in e.get('event_type', '')]),
            'patient_appointments': len([e for e in patient_events if 'appointment' in e.get('event_type', '')]),
            'patient_assessments': len([e for e in patient_events if 'assessment' in e.get('event_type', '')]),
        }
        ...
        if user_activity:
            ...
            metrics['highly_engaged_patients'] = len([u for u in user_activity.values() if u > avg_engagement * 1.5])

        return metrics

    def _analyze_therapy_metrics(self, events):
        """Analyze therapy-related metrics."""
        therapy_events = [e for e in events if 'therapy' in e.get('event_type', '').lower()]

        session_events = [e for e in therapy_events if 'session' in e.get('event_type', '')]
        completed_sessions = [e for e in session_events if 'completed' in e.get('event_type', '')]

        return {
            'total_therapy_events': len(therapy_events),
            'therapy_sessions_started': len([e for e in session_events if 'started' in e.get('event_type', '')]),
            'therapy_sessions_completed': len(completed_sessions),
            'session_completion_rate': round(len(completed_sessions) / max(1, len(session_events)) * 100, 2),
            'avg_sessions_per_day': round(len(session_events) / max(1, (self.config.period_end - self.config.period_start).days), 2)
        }

    def _analyze_communication_metrics(self, events):
        """Analyze communication metrics."""
        comm_events = [e for e in events if any(term in e.get('event_type', '').lower() for term in ['message', 'call', 'notification'])]

        return {
            'total_communications': len(comm_events),
            'messages_sent': len([e for e in comm_events if 'message.sent' in e.get('event_type', '')]),
            'video_calls': len([e for e in comm_events if 'video_call' in e.get('event_type', '')]),
            'notifications_sent': len([e for e in comm_events if 'notification' in e.get('event_type', '')]),
            'avg_communications_per_day': round(len(comm_events) / max(1, (self.config.period_end - self.config.period_start).days), 2)
        }

    def _analyze_operational_metrics(self, events):
        """Analyze operational efficiency metrics."""
        error_events = [e for e in events if 'error' in e.get('event_type', '').lower() or 'failed' in e.get('event_type', '').lower()]
        ...
        return {
            ...
            'failed_logins': len([e for e in auth_events if 'failed' in e.get('event_type', '')]),
            'system_uptime_score': max(0, 100 - (len(error_events) / max(1, len(events)) * 1000))
        }
    ...
    def _analyze_revenue_impact(self, events):
        ...
        appointment_events = [e for e in events if 'appointment' in e.get('event_type', '').lower()]
        completed_appointments = [e for e in appointment_events if 'completed' in e.get('event_type', '')]
        ...
        return {
            ...
            'estimated_revenue': len(completed_appointments) * estimated_revenue_per_appointment,
            'appointment_show_rate': round(len(completed_appointments) / max(1, len(appointment_events)) * 100, 2),
            'revenue_per_day': round((len(completed_appointments) * estimated_revenue_per_appointment) / max(1, (self.config.period_end - self.config.period_start).days), 2)
        }
...
class ScheduledReportService:
    ...
    def generate_daily_summary():
        ...
        config = ReportConfig(
            ...
            period_start=timezone.now().replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=1),
