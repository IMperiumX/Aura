...
class RequestContextFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if request:
            ...
            record.query_string = request.GET.urlencode() if hasattr(request, 'GET') else ''
            record.content_type = request.content_type if hasattr(request, 'content_type') else ''
    ...
    def _add_user_context(self, record: logging.LogRecord, request: WSGIRequest) -> None:
        """
        Adds comprehensive user context to log records.
        """
        if hasattr(request, 'user') and request.user.is_authenticated:
            record.user_id = str(request.user.pk)
            record.username = getattr(request.user, 'username', '') or getattr(request.user, 'email', '')
        ...
        if hasattr(request, 'session'):
            ...
            record.session_age = time.time() - request.session.get('_session_init_timestamp_', time.time())
        else:
            record.session_key = 'no-session'
            record.session_age = 0

    def _add_security_context(self, record: logging.LogRecord, request: WSGIRequest) -> None:
        """
    ...
    def _add_performance_context(self, record: logging.LogRecord, request: WSGIRequest) -> None:
        """
    ...
    def _add_geographic_context(self, record: logging.LogRecord, request: WSGIRequest) -> None:
        """
    ...
    def _add_system_metrics(self, record: logging.LogRecord) -> None:
        ...
        try:
            ...
            # System load
            record.system_load = psutil.getloadavg()[0] if hasattr(psutil, 'getloadavg') else 0
