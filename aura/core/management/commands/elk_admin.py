...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError(f"ELK operation failed: {e!s}")
    ...
    def _health_check(self, options):
        ...
        try:
            ...
            self.stdout.write(
                f"   Initializing Shards: {health['initializing_shards']}",
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                stats_response = requests.get(
                    f"{self.logstash_config['host']}/_node/stats",
                    timeout=10,
                )
                if stats_response.status_code == 200:
                    ...
                    if pipeline_stats:
                        for pipeline_name, pipeline_data in pipeline_stats.items():
                            ...
                            self.stdout.write(
                                f"     Events Out: {events.get('out', 0)}",
                            )
                            self.stdout.write(
                                f"     Events Filtered: {events.get('filtered', 0)}",
                            )
            else:
                self.stdout.write(
                    self.style.ERROR(
                        f"   ✗ Logstash not responding: {response.status_code}",
                    ),
                )
        ...
        try:
            ...
            response = requests.get(
                f"{self.kibana_config['host']}/api/status",
                auth=auth,
                timeout=10,
            )
            ...
            if response.status_code == 200:
                ...
                if overall_status == "available":
                    self.stdout.write(
                        self.style.SUCCESS(f"   ✓ Kibana Status: {overall_status}"),
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(f"   ⚠ Kibana Status: {overall_status}"),
                    )
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"   ✗ Kibana not responding: {response.status_code}",
                    ),
                )
        ...
        try:
            ...
            for index in indices:
                ...
                self.stdout.write(
                    style(
                        f"   {status_icon} {index_name}: {docs_count} docs, {store_size}",
                    ),
                )
                ...
                if "gb" in store_size.lower():
                    total_size += (
                        float(store_size.replace("gb", "").strip()) * 1024 * 1024 * 1024
                    )
                elif "mb" in store_size.lower():
                    total_size += (
                        float(store_size.replace("mb", "").strip()) * 1024 * 1024
                    )
        ...
        for pattern in index_patterns:
            try:
                ...
                if response.status_code in [200, 409]:  # 409 = already exists
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"   ✗ Failed to create pattern {pattern['title']}: {response.status_code}",
                        ),
                    )
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"   ✗ Error creating pattern {pattern['title']}: {e}",
                    ),
                )
    ...
    def _cleanup_indices(self, options):
        ...
        self.stdout.write(
            f"Cleaning up indices older than {days} days ({cutoff_date.strftime('%Y-%m-%d')})...",
        )
        ...
        try:
            ...
            for index_info in indices_to_delete:
                self.stdout.write(
                    f"  - {index_info['name']} ({index_info['date'].strftime('%Y-%m-%d')}) - {index_info['docs']} docs, {index_info['size']}",
                )
            ...
            for index_info in indices_to_delete:
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"   ✓ Deleted: {index_info['name']}"),
                    )
                    deleted_count += 1
                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(
                            f"   ✗ Failed to delete {index_info['name']}: {e}",
                        ),
                    )
    ...
    def _manage_indices(self, options):
        ...
        try:
            ...
            self.stdout.write(
                f"{'Index Name':<40} {'Health':<8} {'Docs':<12} {'Size':<10} {'Status'}",
            )
            self.stdout.write("-" * 80)
            ...
            for index in indices:
                ...
                self.stdout.write(
                    f"{name:<40} {health:<8} {docs:<12} {size:<10} {status}",
                )
    ...
    def _monitor_elk(self, options):
        ...
        try:
            while True:
                ...
                self.stdout.write(
                    f"=== ELK Monitoring - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ===",
                )
                ...
                try:
                    ...
                    self.stdout.write(
                        f"  Documents: {stats['indices']['docs']['count']:,}",
                    )
                    self.stdout.write(
                        f"  Store Size: {stats['indices']['store']['size_in_bytes'] / (1024*1024*1024):.2f} GB",
                    )
        ...
        try:
            ...
            for index in indices:
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"    ✓ Optimized {index_name}"),
                    )

                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(f"    ✗ Failed to optimize {index_name}: {e}"),
                    )
    ...
    def _backup_data(self, options):
        ...
        self.stdout.write(
            "This would create Elasticsearch snapshots for data protection.",
        )
