...
from .filters import PatientFlowEventFilter
...
from .permissions import ClinicAppointmentPermission
from .permissions import ClinicPatientPermission
from .permissions import ClinicPermission
from .permissions import ClinicStatusPermission
from .permissions import FlowEventPermission
from .permissions import NotificationPermission
from .permissions import PatientFlowStaffPermission
from .serializers import AnalyticsResponseSerializer
from .serializers import AppointmentCreateUpdateSerializer
...
from .serializers import FlowBoardSummarySerializer
from .serializers import NotificationSerializer
from .serializers import PatientFlowEventSerializer
from .serializers import PatientFlowPatientSerializer
from .serializers import StatusSerializer
from .serializers import UserProfileSerializer
from .serializers import WeeklyTrendsSerializer
...
class AppointmentViewSet(viewsets.ModelViewSet, AnalyticsRecordingMixin):
    ...
    def bulk_update_status(self, request):
        ...
        if not appointment_ids or not status_id:
            return Response(
                {"error": "appointment_ids and status_id are required"},
                status=status.HTTP_400_BAD_REQUEST,
            )
            ...
            timeline.append(
                {
                    ...
                    "updated_by": event.updated_by.get_full_name()
                    if event.updated_by
                    else None,
                    "notes": event.notes,
...
class AnalyticsViewSet(viewsets.GenericViewSet):
    ...
    def daily_report(self, request):
        ...
        avg_time = (
            sum([d["total_time_minutes"] for d in time_data]) / len(time_data)
            if time_data
            else 0
        )
